...
function buildNavbar(activePage = '') {
  ...
  const navPages = [
    ...
    { key: 'nav_saved',    href: _p('saved.html') },
...
(function updateWishlistBadge() {
  ...
  setTimeout(() => {
    const wl = JSON.parse(localStorage.getItem('gk_wishlist') || '[]');
    const savedLink = document.querySelector('a[href*="saved.html"]');
    if (!savedLink || wl.length === 0) return;
    const badge = document.createElement('span');
    badge.textContent = wl.length;
    badge.style.cssText = 'background:var(--blue);color:white;border-radius:10px;font-size:10px;padding:1px 6px;margin-left:5px;font-weight:700;vertical-align:middle';
